...
- Admit a new indexable article only when its brief names a reader task not already answered by an existing route, the primary or first-party evidence it adds, a net-new conclusion, and why AI Charts is the right publisher. Compare the proposed headings and claims with every current article; consolidate or reject the page when roughly a third overlaps. A source summary plus a product analogy is not enough, and a Hraness reading digest is not independent corroboration of the source it summarizes.
