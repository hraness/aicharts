...
- `app/` – Next.js App Router surface, metadata, error states, and product stylesheet.
- `components/` – interactive scatter chart, linked summaries, sharing, export, and UI primitives.
- `lib/` – strict data boundaries, chart math, layout, analytics, and property tests.
...
- Keep CodingChart a chart-only Next.js product with a checked snapshot and no runtime dependency on the upstream benchmark page.
...
- Update `data/coding-agents.json` only through `bun run data:refresh`; preserve duplicate, retention, stable-key, and metric-coverage guards.
