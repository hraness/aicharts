...
Selective forward ports beyond the pin: `src/clients.rs` tracks upstream commit `3854b9d2dbc550b178c97caab2f9d5cc7193217a` byte-for-byte (adds `antigravity-extension`); `src/sessions/micode.rs`, `src/sessions/muse.rs` and `src/sessions/micode_io_tests.rs` track upstream commit `fe584950f81b2471a3dc23e8ec8117c84a552822` byte-for-byte (upstream `b510f9c` Xiaomi MiMo desktop split, `d409ee0` Muse Code client, `fe58495` MiMo/MiniMax/Muse accounting corrections); their UPSTREAM-SHA256SUMS entries record that commit's bytes. The corresponding hunks were merged into the aicharts-delta files `src/lib.rs`, `src/scanner.rs`, `src/message_cache.rs`, `src/provider_identity.rs`, `src/sessions/mod.rs`, `src/sessions/antigravity_cli.rs`, `src/sessions/dsh.rs`, `src/sessions/mcode.rs`, `src/sessions/openclaw.rs`, `src/sessions/codex.rs` and `src/sessions/opencode_schema.rs` by patch application, covering upstream `3854b9d` (Antigravity IDE extension), `07db1c2` + `58741d5` (dsh embedded attempts and mixed-shard test), `dc8222f` (MiniMax stream-json envelopes), `1d9a939` (OpenClaw compressed SQLite payloads) and `8d09044` (Codex Fast/Priority service tier), leaving local modifications intact. Upstream commits `2edc986` (import backfill CLI), `7ebc3e8` (TUI settings errors), `b7209fb` (TUI localization) and `23d62d0` (coverage badge) touch only upstream CLI/CI surfaces that are not vendored.

Upstream `1436912` (`src/recovery.rs`, a local disaster-recovery overlay that injects `~/.config/tokscale/recovered-usage-v1.json` rows into the display path) is deliberately not ported: it would read a root outside the admitted explicit-source profiles and inject non-source messages into parsed output. The offline collection path must keep admitting only explicitly approved roots; if the overlay is ever needed it belongs behind its own admitted source profile, not an implicit config-dir read.
