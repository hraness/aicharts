...
pub(crate) fn parse_jsonc(contents: &str) -> Option<serde_json::Value> {
    crate::offline_io::json_str(contents)
        .ok()
        .or_else(|| crate::offline_io::json_str(&strip_jsonc_syntax(contents)).ok())
}
...
fn load_with_environment(
    ...
    for filename in ["opencode.json", "opencode.jsonc"] {
        if let Ok(contents) = crate::offline_io::read_to_string(config_dir.join(filename)) {
    ...
    if let Some(path) = environment.and_then(|config| config.custom_config.as_deref()) {
        if let Ok(contents) = crate::offline_io::read_to_string(path) {
            resolver.extend_from_json(&contents);
        }
    }
    ...
    for filename in ["opencode.json", "opencode.jsonc"] {
        if let Ok(contents) = crate::offline_io::read_to_string(legacy_dir.join(filename)) {
            resolver.extend_from_json(&contents);
        }
    }
