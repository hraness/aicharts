...
pub fn parse_commandcode_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for_each_json_line(path, &mut |_index, trimmed| {
        let entry = match crate::offline_io::json_str::<CommandCodeEntry>(trimmed) {
            Ok(entry) => entry,
            Err(_) => return,
        };
        ...
        match entry.record_type.as_deref() {
            ...
            _ => {
                ...
                if role == "assistant" {
                    ...
                    message.tokens_estimated = tokens_estimated;
...
fn model_from_config(session_path: &Path) -> Option<String> {
    ...
    let bytes = crate::offline_io::read(config_path).ok()?;
    let config: CommandCodeConfig = crate::offline_io::json_slice(&bytes).ok()?;
