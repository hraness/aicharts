...
pub fn parse_jcode_file(path: &Path) -> Vec<UnifiedMessage> {
    let mut data = match crate::offline_io::read(path) {
        Ok(data) => data,
        Err(_) => return Vec::new(),
    };
    let envelope: JcodeSessionEnvelope = match crate::offline_io::simd_slice(&mut data) {
    ...
    for_each_json_line(&journal_path, &mut |line_index, trimmed| {
        let Ok(entry) = crate::offline_io::json_str::<JcodeJournalEntry>(trimmed) else {
            return;
        };
