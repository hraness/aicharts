...
pub fn parse_grok_updates_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for line in lossy_lines(BufReader::new(file)) {
        ...
        let Ok(value) = crate::offline_io::json_str::<Value>(&line) else {
            continue;
        };
...
fn parse_grok_unified_log_snapshot(
    path: &Path,
    file: &mut crate::offline_io::ReadFile,
    prefix_len: u64,
) -> Vec<UnifiedMessage> {
...
fn unified_session_tree(path: &Path) -> Option<(PathBuf, UnifiedSessionTree)> {
    ...
    let Ok(entries) = crate::offline_io::read_dir(&sessions_root) else {
    ...
    for entry in entries.flatten() {
        ...
        if let Ok(entries) = crate::offline_io::read_dir(&workspace_dir) {
            for entry in entries.flatten() {
...
fn append_signals_reconciliation(
    ...
    let value: Value = match crate::offline_io::json_slice(&data) {
        Ok(value) => value,
        Err(_) => return,
    };
...
fn read_signals_metadata(path: &Path, metadata: &mut GrokMetadata) {
    ...
    let Ok(value) = crate::offline_io::json_slice::<Value>(&data) else {
        return;
    };
