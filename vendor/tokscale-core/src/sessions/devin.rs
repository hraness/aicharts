...
pub fn parse_devin_cli_sqlite(db_path: &Path) -> Vec<UnifiedMessage> {
    ...
    // This source records independent responses. An earlier write cannot
    // back-anchor forward into the requested range. Do not use an upper bound:
...
mod tests {
    ...
    fn report_lower_bound_keeps_late_writes_that_back_anchor_into_the_window() {
        ...
        let db = std::fs::canonicalize(db).unwrap();
        ...
        insert_message(&conn, "one", row, 1_700_000_000);
        insert_message(&conn, "one", row, 1_700_000_010);
        insert_message(&conn, "one", row, 1_700_000_020);
        drop(conn);
        let complete = parse_devin_cli_sqlite(&db);
        let guard = crate::offline_io::begin(std::slice::from_ref(&root)).unwrap();
        crate::offline_io::set_first_observed_ms(Some(1_700_000_005_000));
        let bounded = parse_devin_cli_sqlite(&db);
        guard.finish().unwrap();
        assert_eq!(
            bounded,
            complete
                .into_iter()
                .filter(|row| row.timestamp >= 1_700_000_005_000)
                .collect::<Vec<_>>()
        );
