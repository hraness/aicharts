...
pub struct UnifiedMessage {
    ...
    /// AI Charts local provenance: a bucket is estimated or not fully verified.
    #[serde(default)]
...
impl UnifiedMessage {
    ...
    fn new_full(
        ...
        crate::offline_io::observe_row();
        let client = client.into();
        // Kiro mixes native counters and content fallbacks; conservatively keep
        // its rows out of the fully reported total pending per-field provenance.
        let tokens_estimated = matches!(client.as_str(), "kiro" | "freebuff");
...
fn read_git_pointer_file(path: &Path) -> Option<String> {
    // A numeric import does not need to inspect arbitrary project metadata.
...
pub fn decode_claude_project_slug(key: &str) -> Option<String> {
    // Keep the recorded slug as local metadata instead of walking from the
    // filesystem root to discover a prettier display label.
    if crate::offline_io::active() {
        return None;
    }
...
fn resolve_slug_under(dir: &Path, remaining: &str, budget: &mut u32) -> SlugResolution {
    ...
    let Ok(entries) = crate::offline_io::read_dir(dir) else {
