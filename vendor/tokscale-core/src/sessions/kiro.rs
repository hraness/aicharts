...
pub fn parse_kiro_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let mut json_bytes = match crate::offline_io::read(path) {
        Ok(bytes) => bytes,
        Err(_) => return Vec::new(),
    };

    let header = match crate::offline_io::simd_slice::<KiroSessionHeader>(&mut json_bytes) {
        Ok(header) => header,
        Err(_) => return Vec::new(),
    };
    ...
    for_each_json_line(&jsonl_path, &mut |_index, trimmed| {
        ...
        // kinds.
        let raw = match crate::offline_io::json_str::<Value>(trimmed) {
            Ok(value) => value,
            Err(_) => return,
        };
        ...
        let entry = match crate::offline_io::simd_slice::<KiroJsonlEntry>(&mut bytes) {
            Ok(entry) => entry,
            Err(_) => return,
        };
...
fn parse_kiro_ide_session_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let session_json = match crate::offline_io::read_to_string(path) {
        Ok(contents) => contents,
        Err(_) => return Vec::new(),
    };
    let session: KiroIdeSession = match crate::offline_io::json_str(&session_json) {
        Ok(session) => session,
        Err(_) => return Vec::new(),
    };
    ...
    for_each_json_line(&messages_path, &mut |_index, trimmed| {
        let entry: Value = match crate::offline_io::json_str(trimmed) {
            Ok(v) => v,
            Err(_) => return,
        };
...
fn parse_kiro_global_storage_file(path: &Path) -> Vec<UnifiedMessage> {
    let fallback_timestamp = file_modified_timestamp_ms(path);
    let json = match crate::offline_io::read_to_string(path) {
        Ok(contents) => contents,
        Err(_) => return Vec::new(),
    };

    let value: Value = match crate::offline_io::json_str(&json) {
        Ok(value) => value,
        Err(_) => return Vec::new(),
    };
...
pub fn parse_kiro_sqlite(db_path: &Path) -> Vec<UnifiedMessage> {
    ...
    sqlite_for_each_row(db_path, query, Some("Kiro conversation"), &mut |row| {
        ...
        let parsed = match crate::offline_io::json_str::<KiroDbConversation>(&json_str) {
            Ok(p) => p,
            Err(_) => return Ok(()),
        };
...
{"version":"v1","kind":"AssistantMessage","data":{"message_id":"assistant-3","content":[{"kind":"text","data":"response text"}]}}"#;
    ...
    fn test_collect_kiro_snapshot_text_does_not_double_count_aliased_keys() {
        ...
        let value: Value = crate::offline_io::json_str(
            r#"{
                "messages": [
    ...
    fn test_find_kiro_snapshot_model_id_descends_into_aliased_text_keys() {
        ...
        let parts_value: Value = crate::offline_io::json_str(
            r#"{
                "messages": [
        ...
        let prompt_value: Value =
            crate::offline_io::json_str(r#"{"prompt": {"model": "claude-sonnet-4"}}"#).unwrap();
        assert_eq!(
