...
fn extract_model_from_jsonl(jsonl_path: &Path) -> Option<String> {
    let file = crate::offline_io::open(jsonl_path).ok()?;
...
fn transcript_turns_bounded(jsonl: &Path, max_bytes: u64, max_turns: usize) -> Vec<TranscriptTurn> {
    let Ok(file) = crate::offline_io::open(jsonl) else {
        return Vec::new();
    };
    ...
    for line in lossy_lines(&mut reader) {
        ...
        let Ok(record) = crate::offline_io::json_str::<Value>(&line) else {
...
pub fn parse_droid_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let mut bytes = data;
    let settings: DroidSettingsJson = match crate::offline_io::simd_slice(&mut bytes) {
        Ok(s) => s,
...
mod tests {
    ...
    fn test_parse_droid_settings_structure() {
        ...
        let settings: DroidSettingsJson = crate::offline_io::simd_slice(&mut bytes).unwrap();
