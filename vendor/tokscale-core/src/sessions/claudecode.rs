...
fn resolve_subagent_name(
    ...
    if let Ok(text) = crate::offline_io::read_to_string(&meta_path) {
        if let Ok(meta) = crate::offline_io::json_str::<AgentMetaFile>(&text) {
...
fn parent_session_paths_for_cache_bounded(
    ...
    let Ok(file) = crate::offline_io::open(sidechain_path) else {
        return Vec::new();
    };
    ...
    loop {
        ...
        if let Ok(entry) = crate::offline_io::json_str::<ClaudeEntry>(line.trim_end()) {
...
fn build_parent_subagent_type_lookup(parent_path: &Path) -> Option<HashMap<String, String>> {
    let file = crate::offline_io::open(parent_path).ok()?;
    ...
    for line in reader.lines() {
        ...
        let value: serde_json::Value = match crate::offline_io::json_str(trimmed) {
            Ok(v) => v,
            Err(_) => {
                crate::offline_io::fault("import_read_failed");
                continue;
...
fn extract_claude_tool_result_message(
    ...
    let value: Value = crate::offline_io::json_str(line).ok()?;
...
fn parse_claude_headless_json(
    ...
    let mut bytes = data;
    let value: Value = match crate::offline_io::simd_slice(&mut bytes) {
        Ok(v) => v,
        Err(_) => return Vec::new(),
    };
...
fn process_claude_headless_line(
    ...
    let value: Value = crate::offline_io::simd_slice(&mut bytes).ok()?;
