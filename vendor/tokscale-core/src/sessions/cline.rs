...
fn read_cline_cli_manifest(path: &Path) -> ClineCliManifest {
    ...
    let Ok(mut bytes) = crate::offline_io::read(manifest_path) else {
        return ClineCliManifest::default();
    };

    crate::offline_io::simd_slice(&mut bytes).unwrap_or_default()
}
...
pub fn parse_cline_cli_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    let mut bytes = data;
    let Ok(file) = crate::offline_io::simd_slice::<ClineCliMessagesFile>(&mut bytes) else {
        return Vec::new();
    };
