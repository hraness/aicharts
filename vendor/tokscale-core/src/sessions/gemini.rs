...
pub fn gemini_session_id_for_file(path: &Path) -> Option<String> {
    ...
    if path.extension().and_then(|s| s.to_str()) == Some("jsonl") {
        ...
        for line in content.lines() {
            ...
            if let Ok(value) = crate::offline_io::json_str::<Value>(trimmed) {
                if let Some(id) =
    ...
    if let Ok(value) = crate::offline_io::json_str::<Value>(&content) {
...
pub(crate) fn parse_gemini_file_with_cache_status(path: &Path) -> GeminiParseResult {
    ...
    if let Ok(session) = crate::offline_io::simd_slice::<GeminiSession>(&mut bytes) {
        return GeminiParseResult {
    ...
    let mut bytes = data;
    if let Ok(value) = crate::offline_io::simd_slice::<Value>(&mut bytes) {
        let session_id = path
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("unknown")
...
fn parse_gemini_headless_jsonl(path: &Path, fallback_timestamp: i64) -> GeminiParseResult {
    ...
    loop {
        ...
        let value: Value = match crate::offline_io::simd_slice(&mut json_buffer) {
            Ok(v) => v,
            Err(_) => {
                skipped_malformed_line = true;
                continue;
...
mod tests {
    ...
    fn test_parse_gemini_structure() {
        ...
        let session: GeminiSession = crate::offline_io::simd_slice(&mut bytes).unwrap();
