...
fn parse_codex_reader<R: BufRead>(
    ...
    loop {
        line.clear();
        let bytes_read =
            match std::io::Read::take(&mut reader, crate::offline_io::MAX_LINE_BYTES as u64 + 1)
                .read_line(&mut line)
            {
                Ok(0) => break,
                Ok(bytes_read) => bytes_read,
                Err(_) => {
                    crate::offline_io::fault("import_read_failed");
                    parse_succeeded = false;
                    break;
                }
            };
        if bytes_read > crate::offline_io::MAX_LINE_BYTES {
            crate::offline_io::fault("import_line_limit");
            parse_succeeded = false;
            break;
        }
        ...
        let parsed_entry = crate::offline_io::simd_slice::<CodexEntry>(&mut buffer);
        let valid_entry_json = parsed_entry.is_ok();
        if let Ok(entry) = parsed_entry {
        ...
        if state.forked_child_waiting_for_turn_context {
            ...
            if valid_entry_json
                || crate::offline_io::simd_slice::<serde::de::IgnoredAny>(&mut json_probe).is_ok()
            {
                continue;
            }
        }
        ...
        let mut json_probe = trimmed.as_bytes().to_vec();
        if !valid_entry_json
            && crate::offline_io::simd_slice::<serde::de::IgnoredAny>(&mut json_probe).is_err()
        {
            parse_succeeded = false;
            continue;
        }
...
pub fn parse_codex_file(path: &Path) -> Vec<UnifiedMessage> {
    let file = match crate::offline_io::open(path) {
        Ok(f) => f,
...
pub(crate) fn parse_codex_file_incremental(
    ...
    let mut file = match crate::offline_io::open(path) {
        Ok(file) => file,
        Err(_) => {
...
fn parse_codex_headless_line(
    ...
    impl<'de> serde::Deserialize<'de> for Field {
        fn deserialize<D: serde::Deserializer<'de>>(d: D) -> Result<Self, D::Error> {
            Value::deserialize(d).map(|value| Self(Some(value)))
        }
    }
    #[derive(Deserialize, Default)]
    struct HeadlessFields {
        #[serde(default)]
        model: Field,
        #[serde(default)]
        model_name: Field,
    ...
    let fields: HeadlessFields = crate::offline_io::json_str(line).ok()?;
    let value = Value::Object(
        ...
        .into_iter()
        .filter_map(|(key, field)| field.0.map(|value| (key.to_owned(), value)))
        .collect(),
    );
...
mod tests {
    ...
    #[test]
    fn large_rollout_payload_does_not_hide_later_numeric_usage() {
        let payload = "x".repeat(9 * 1024 * 1024);
        let content = format!("{{\"type\":\"response_item\",\"payload\":{{\"type\":\"function_call_output\",\"output\":\"{payload}\"}}}}\n{{\"type\":\"turn.completed\",\"model\":\"gpt-4o-mini\",\"usage\":{{\"input_tokens\":120,\"cached_input_tokens\":20,\"output_tokens\":30}}}}\n");
        let file = create_test_file(&content);
        ...
        assert_eq!(parsed.messages[0].tokens.output, 30);
        assert_eq!(parsed.messages[0].tokens.cache_read, 20);
    }
