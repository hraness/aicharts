...
pub fn parse_hindsight_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    lossy_lines(BufReader::new(file))
        .filter_map(|line| {
            ...
            let record: HindsightRecord = crate::offline_io::json_str(line).ok()?;
            ...
            let mut input = record.input_tokens.unwrap_or(0).max(0);
            ...
            if cache_read > 0 {
                ...
                let disjoint_total = inclusive_total.and_then(|sum| sum.checked_add(cache_read));
                if record.total_tokens == inclusive_total && input >= cache_read {
                    input -= cache_read;
                } else if record.total_tokens != disjoint_total || disjoint_total.is_none() {
                    crate::offline_io::fault("import_token_semantics_invalid");
                    return None;
                }
            }
            let cache_write = 0;
