...
pub(crate) fn parse_roo_kilo_file(path: &Path, source: &str) -> Vec<UnifiedMessage> {
    ...
    let mut bytes = data;
    let entries: Vec<UiMessageEntry> = match crate::offline_io::simd_slice(&mut bytes) {
        Ok(v) => v,
...
fn read_task_metadata(ui_messages_path: &Path) -> (String, Option<String>) {
    ...
    let content = match crate::offline_io::read_to_string(&history_path) {
        Ok(c) => c,
...
fn parse_api_req_started_payload(text: &str) -> Option<ApiReqStartedPayload> {
    ...
    let cost = extract_f64(value.get("cost")).filter(|value| value.is_finite() && *value >= 0.0);
