...
fn read_line_payload<R: BufRead>(
        ...
        .read_until(b'\n', buf)
    {
        Ok(0) => None,
        Ok(_) => {
            if buf.len() > crate::offline_io::MAX_LINE_BYTES {
                crate::offline_io::fault("import_line_limit");
                return None;
            }
            if std::str::from_utf8(buf).is_err() {
                crate::offline_io::fault("import_invalid_utf8");
            }
            if buf.last() == Some(&b'\n') {
                buf.pop();
...
pub(crate) fn for_each_json_line(path: &Path, sink: &mut dyn FnMut(usize, &str)) {
    let Ok(file) = crate::offline_io::open(path) else {
        return;
    };
...
pub(crate) fn file_modified_timestamp_ms_opt(path: &Path) -> Option<i64> {
    crate::offline_io::metadata(path)
...
pub(crate) fn sqlite_for_each_row_on_with_params(
    ...
    let mut rows = match stmt.query(params) {
        ...
        Err(err) => {
            ...
            crate::offline_io::fault("import_sqlite_query_failed");
        ...
        match rows.next() {
            Ok(Some(row)) => {
                if let Err(err) = sink(row) {
                    crate::offline_io::fault("import_sqlite_row_invalid");
                    if let Some(what) = what {
                        warn!(
                            db_path = %db_path.display(),
                            what,
            ...
            Err(err) => {
                ...
                crate::offline_io::fault("import_sqlite_read_failed");
                stepped_off = true;
                break;
            }
        }
    ...
    if stepped_off {
        ...
        crate::offline_io::sqlite_completed(db_path);
        SqliteScan::Ran
    }
}
...
pub(crate) fn parse_json_line<T: DeserializeOwned>(line: &str, buffer: &mut Vec<u8>) -> Option<T> {
    ...
    crate::offline_io::simd_slice(buffer).ok()
}
