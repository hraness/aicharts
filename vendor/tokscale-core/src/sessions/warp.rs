...
pub fn parse_warp_file(path: &Path) -> Vec<UnifiedMessage> {
    let content = match crate::offline_io::read_to_string(path) {
        Ok(content) => content,
        Err(_) => return Vec::new(),
    };
    let cache: WarpUsageCache = match crate::offline_io::json_str(&content) {
        Ok(cache) => cache,
        Err(_) => return Vec::new(),
    };
    let timestamp = cache
...
fn usage_to_message(usage: &WarpAggregateUsage, timestamp: i64) -> Option<UnifiedMessage> {
    ...
    if usage.spend_cents.is_some_and(|value| value >= 0) {
...
fn workspace_to_message(workspace: &WarpWorkspaceUsage, timestamp: i64) -> Option<UnifiedMessage> {
    ...
    if workspace.spend_cents.is_some_and(|value| value >= 0) {
        message.mark_provider_reported_cost();
    }
    message.message_count = requests;
