...
fn read_model_from_config(wire_path: &Path) -> String {
    if let Some(config_path) = kimi_config_path(wire_path) {
        if let Ok(content) = crate::offline_io::read_to_string(&config_path) {
            if let Ok(bytes) = crate::offline_io::json_str::<serde_json::Value>(&content) {
...
fn read_workspaces_index(root: &Path) -> HashMap<String, KimiCodeWorkspace> {
    ...
    let Ok(contents) = crate::offline_io::read_to_string(root.join("workspaces.json")) else {
        return by_slug;
    };
    let Ok(value) = crate::offline_io::json_str::<serde_json::Value>(&contents) else {
        return by_slug;
    };
...
fn read_session_index(root: &Path) -> HashMap<String, KimiCodeWorkspace> {
    ...
    let Ok(contents) = crate::offline_io::read_to_string(root.join("session_index.jsonl")) else {
        return by_session;
    };
    for line in contents.lines() {
        let Ok(value) = crate::offline_io::json_str::<serde_json::Value>(line) else {
            continue;
        };
        let workspace = value
            .get("workDir")
...
pub fn parse_kimi_code_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for_each_json_line(path, &mut |_index, trimmed| {
        ...
        let wire_line = match crate::offline_io::simd_slice::<KimiCodeWireLine>(&mut bytes) {
            Ok(wl) => wl,
            Err(_) => return,
        };
...
pub fn parse_kimi_file(path: &Path) -> Vec<UnifiedMessage> {
    ...
    for_each_json_line(path, &mut |_index, trimmed| {
        let mut bytes = trimmed.as_bytes().to_vec();
        let wire_line = match crate::offline_io::simd_slice::<WireLine>(&mut bytes) {
            Ok(wl) => wl,
            Err(_) => return,
        };
