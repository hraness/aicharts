...
pub fn parse_opencode_file(path: &Path) -> Option<UnifiedMessage> {
    ...
    let msg: OpenCodeMessage = crate::offline_io::simd_slice(&mut bytes).ok()?;
...
pub fn load_opencode_migration_cache() -> Option<OpenCodeMigrationCache> {
    ...
    match crate::offline_io::read_to_string(&canonical) {
        Ok(content) => crate::offline_io::json_str(&content).ok(),
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            legacy_migration_cache_paths().into_iter().find_map(|path| {
                let content = crate::offline_io::read_to_string(path).ok()?;
                crate::offline_io::json_str(&content).ok()
            })
        }
        Err(_) => None,
    }
...
mod tests {
    ...
    fn test_deserialize_v2_message_resolves_nested_model() {
        ...
        let msg: OpenCodeMessage = crate::offline_io::simd_slice(&mut bytes).unwrap();
    ...
    fn test_migration_cache_round_trip() {
        ...
        let loaded: OpenCodeMigrationCache = crate::offline_io::json_str(&json).unwrap();
    ...
    fn test_migration_cache_missing_returns_none() {
        ...
        // crate::offline_io::json_str returns None for invalid input (simulating missing file).
        let result: Option<OpenCodeMigrationCache> = crate::offline_io::json_str("").ok();
        assert!(
            result.is_none(),
