...
const OPENCLAW_SQLITE_BUSY_TIMEOUT: Duration = Duration::from_millis(100);
...
fn read_archive(path: &Path, max_bytes: u64) -> std::io::Result<Vec<u8>> {
    let file = crate::offline_io::open(path)?;
...
fn for_each_transcript_line(path: &Path, sink: &mut dyn FnMut(usize, &str)) {
    ...
    match read_archive(path, MAX_ARCHIVE_BYTES) {
        ...
        Err(error) => {
            crate::offline_io::fault("import_compression_invalid");
...
pub fn parse_openclaw_index(index_path: &Path) -> Vec<UnifiedMessage> {
    ...
    let mut bytes = data;
    let index: SessionIndex = match crate::offline_io::simd_slice(&mut bytes) {
        Ok(i) => i,
...
pub(crate) fn scan_openclaw_sqlite(db_path: &Path) -> OpenClawSqliteScan {
    let conn = match open_readonly_sqlite(db_path) {
        Ok(conn) => conn,
        Err(err) => {
            crate::offline_io::fault("import_sqlite_open_failed");
            warn!(
...
pub(crate) mod test_fixtures {
    ...
    pub(crate) fn insert_event(
        ...
        .unwrap();
        if let Ok(value) = crate::offline_io::json_str::<serde_json::Value>(event_json) {
