...
const PER_MODEL_QUERY: &str = r#"
        SELECT
            ...
            SUM(CASE WHEN smu.actual_cost_usd > 0 OR (COALESCE(smu.actual_cost_usd, 0) = 0 AND smu.estimated_cost_usd >= 0) THEN 1 ELSE 0 END) AS known_cost_rows,
            COUNT(*) AS cost_rows,
            MAX(CASE WHEN COALESCE(smu.actual_cost_usd, 0) = 0 AND smu.estimated_cost_usd >= 0 THEN 1 ELSE 0 END) AS has_estimate
...
const SESSION_TOTALS_QUERY: &str = r#"
        SELECT
            ...
            CASE WHEN actual_cost_usd >= 0 OR (actual_cost_usd IS NULL AND estimated_cost_usd >= 0) THEN 1 ELSE 0 END AS known_cost_rows,
            1 AS cost_rows,
            CASE WHEN actual_cost_usd IS NULL AND estimated_cost_usd >= 0 THEN 1 ELSE 0 END AS has_estimate
        FROM sessions
...
struct HermesUsageRow {
    ...
    reasoning: i64,
    cost: f64,
    cost_source: super::CostSource,
}

fn decode_usage_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<HermesUsageRow> {
    let cost = row.get::<_, Option<f64>>(10)?.unwrap_or(0.0);
    let known: i64 = row.get(11)?;
    let count: i64 = row.get(12)?;
    let estimated: i64 = row.get(13)?;
    let cost_source = if !cost.is_finite() || cost < 0.0 || known != count {
        super::CostSource::Unknown
    } else if estimated > 0 {
...
fn build_message(row: HermesUsageRow, dedup_key: String) -> UnifiedMessage {
    ...
    msg.cost_source = row.cost_source;
