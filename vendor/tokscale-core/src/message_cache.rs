...
impl RelatedFileFingerprint {
    fn from_path(suffix: String, path: &Path, mode: ContentHashMode) -> Option<Self> {
        ...
        match crate::offline_io::metadata(path) {
            Ok(_) => {
...
fn metadata_signature(path: &Path) -> std::io::Result<(u64, u64)> {
    let metadata = crate::offline_io::metadata(path)?;
...
fn compute_sample_hashes(path: &Path, size: u64) -> Option<Vec<FileSampleHash>> {
    if crate::offline_io::metadata(path).ok()?.is_dir() {
        return Some(Vec::new());
    }
    if size == 0 {
        return Some(Vec::new());
    ...
    let mut file = crate::offline_io::open(path).ok()?;
...
fn file_fingerprint_parts(
    ...
    let metadata = crate::offline_io::metadata(path).ok()?;
