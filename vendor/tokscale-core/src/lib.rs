...
fn parse_all_messages_streaming<S: MessageSink>(
    ...
    fn parse_full_log_source(
        ...
        if crate::offline_io::active() {
            ...
            // source cache. Do not clone all messages or reread the transcript
            // to construct a fingerprint that would immediately be discarded.
    ...
    if include_devin_desktop {
        ...
        // Offline reads never create source fingerprints. Capture one lookup
        // per invocation, shared across its parallel Desktop files; every
        // database is still read through its own read-only transaction.
        let offline_devin_desktop_lookup = OnceLock::new();
        let devin_desktop_outcomes: Vec<CachedParseOutcome> = scan_result
            ...
            .map(|path| {
                load_or_parse_source_with_fingerprint_context(
                    ...
                    |path, fingerprint| {
                        if crate::offline_io::active() {
                            let lookup = offline_devin_desktop_lookup.get_or_init(|| {
