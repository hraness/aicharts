...
mod profile;
pub(crate) use profile::synthetic_dbs as profile_synthetic_dbs;
...
fn prime_agent_session_dir_from_settings_files(
    ...
    fn read_session_dir(path: &Path) -> Option<Option<String>> {
        let content = crate::offline_io::read_to_string(path).ok()?;
        let settings: Value = crate::offline_io::json_str(&content).ok()?;
...
pub fn scan_directory(root: &str, pattern: &str) -> Vec<PathBuf> {
    if crate::offline_io::active() && crate::offline_io::admit(Path::new(root)).is_err() {
        return Vec::new();
    }
    ...
    let mut paths: Vec<PathBuf> = WalkDir::new(root)
        ...
        .filter_entry(|e| {
            if crate::offline_io::active() {
                if e.depth() == 64 && e.file_type().is_dir() {
                    crate::offline_io::fault("import_directory_depth_limit");
                    return false;
                }
                if !crate::offline_io::entry(e.path()) {
                    return false;
                }
            }
        ...
        .filter_map(|e| match e {
            Ok(e) => Some(e),
            Err(_) => {
                crate::offline_io::fault("import_directory_unreadable");
                None
            }
        })
        .filter(|e| {
...
fn omo_task_state(omo_dir: &Path) -> OmoTaskState {
    for filename in ["omo.jsonc", "omo.json"] {
        let Ok(contents) = crate::offline_io::read_to_string(omo_dir.join(filename)) else {
            continue;
        };
...
fn discover_senpi_omo_children_roots(
    ...
    let entries = match crate::offline_io::read_dir(sessions_root) {
        Ok(entries) => entries,
...
fn senpi_project_cwds_from_session_dir(session_dir: &Path) -> Vec<PathBuf> {
    let Ok(entries) = crate::offline_io::read_dir(session_dir) else {
        return Vec::new();
    };
...
fn senpi_session_header_cwd(path: &Path) -> Option<PathBuf> {
    ...
    let file = crate::offline_io::open(path).ok()?;
    let mut first_line = String::new();
    BufReader::new(file.take(SENPI_SESSION_HEADER_MAX_BYTES))
        .read_line(&mut first_line)
        .ok()?;

    let header: Value = crate::offline_io::json_str(first_line.trim()).ok()?;
...
pub(crate) fn discover_hermes_profile_state_dbs(hermes_home: &Path) -> Vec<PathBuf> {
    ...
    let mut dbs: Vec<PathBuf> = crate::offline_io::read_dir(hermes_home.join("profiles"))
        .into_iter()
...
pub fn discover_openclaw_agent_dbs(agents_root: &Path) -> Vec<PathBuf> {
    let entries = match crate::offline_io::read_dir(agents_root) {
        Ok(entries) => entries,
        Err(_) => return Vec::new(),
    };
...
pub(crate) fn discover_opencode_dbs(data_dir: &Path) -> Vec<PathBuf> {
    let entries = match crate::offline_io::read_dir(data_dir) {
        Ok(entries) => entries,
        Err(_) => return Vec::new(),
    };
...
fn scan_crush_registry(registry_path: &Path) -> Vec<CrushDbSource> {
    let registry = match crate::offline_io::read_to_string(registry_path) {
        Ok(contents) => contents,
        Err(_) => return Vec::new(),
    };

    let list: CrushProjectList = match crate::offline_io::json_str(&registry) {
        Ok(list) => list,
        Err(_) => return Vec::new(),
    };
...
fn kimi_work_share_dir_root(app_data: &Path) -> Option<PathBuf> {
    ...
    let content = crate::offline_io::read_to_string(config_path).ok()?;
    let config: Value = crate::offline_io::json_str(&content).ok()?;
...
fn discover_copilot_vscode_sessions(home_dir: &str, use_env_roots: bool) -> Vec<PathBuf> {
    ...
    for workspace_storage in &roots {
        let hash_dirs = match crate::offline_io::read_dir(workspace_storage) {
            Ok(rd) => rd,
            Err(_) => continue,
        };
        for entry in hash_dirs.filter_map(|e| e.ok()) {
            ...
            let chat_entries = match crate::offline_io::read_dir(&chat_sessions_dir) {
                Ok(rd) => rd,
                Err(_) => continue,
            };
...
fn scan_all_clients_with_env_strategy_inner(
    ...
    // An AI Charts source profile replaces discovery; it never adds defaults.
    if let Some((client, roots)) = crate::offline_io::profile() {
        return profile::scan(&client, &roots);
    }
...
mod tests {
    ...
    fn test_scanner_settings_deserialize_from_json_camel_case() {
        ...
        let parsed: ScannerSettings = crate::offline_io::json_str(json).unwrap();
        ...
        let empty: ScannerSettings = crate::offline_io::json_str("{}").unwrap();
