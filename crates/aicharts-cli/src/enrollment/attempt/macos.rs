//! Private macOS/APFS implementation of the attempt persistence port.
//! Every writer must cooperate with the stable lock. Atomic rename under that
//! lock is not a filesystem content-CAS against hostile same-user writers.
//! No constructor establishes durable evidence or enrollment/custody authority.
//! Descriptor/ACL/flush rules follow the existing custody reference adapter.
mod anchor;
#[cfg(test)]
mod tests;
...
const DIRECTORY: &str = "enrollment-attempt-v1";
const LOCK: &str = "attempt.lock";
const CURRENT: &str = "attempt.current";
const PENDING: &str = "attempt.pending";
...
impl MacStorage {
    ...
    /// Validates the proposed initial record before path traversal or creation.
    /// The caller must still explicitly initialize through the persistence core.
    pub(super) fn create_new(path: &std::path::Path, initial: &record::Record) -> Result<Self> {
        record::initial(initial)?;
        Self::from_path(path, true)
    }
    /// Existing-only inspection of layout; never creates, adopts or repairs it.
    pub(super) fn open_existing(path: &std::path::Path) -> Result<Self> {
        Self::from_path(path, false)
    }
    #[cfg(test)]
    fn create_new_at(anchor: OwnedFd, initial: &record::Record) -> Result<Self> {
        record::initial(initial)?;
        Self::construct(anchor, true, None)
    }
    #[cfg(test)]
    fn open_existing_at(anchor: OwnedFd) -> Result<Self> {
        Self::construct(anchor, false, None)
    }

    fn from_path(path: &std::path::Path, create: bool) -> Result<Self> {
    ...
    fn read_named(&mut self, name: &str) -> Result<Option<Stored>> {
        ...
        if before.st_size > disk::MAX_BYTES as i64 {
            return Err(Error::RecoveryRequired);
        }
        ...
        self.locked_edges()?;
        let candidate = disk::decode(&bytes)?;
        Ok(Some(Stored {
            fd,
    ...
    fn predecessor(current: Option<&Stored>, candidate: &disk::Decoded) -> Result<()> {
        match (current, candidate.expected) {
            (None, None) => Ok(()),
            (Some(current), Some(expected)) => {
                if !expected.matches(current.candidate.next) {
                    return Err(Error::StaleSnapshot);
                }
                let old = record::decode(current.candidate.bytes.as_bytes())
                    .map_err(|_| Error::RecoveryRequired)?;
                let new = record::decode(candidate.bytes.as_bytes())
                    .map_err(|_| Error::RecoveryRequired)?;
                record::successor(&old, &new).map_err(|_| Error::RecoveryRequired)
            }
...
impl Storage for MacStorage {
    ...
    fn read_committed(&mut self, max_bytes: usize) -> Result<Option<Vec<u8>>> {
        ...
        match current {
            Some(value) if value.candidate.bytes.as_bytes().len() > max_bytes => {
                Err(Error::InvalidRecord)
            }
            Some(value) => Ok(Some(value.candidate.bytes.as_bytes().to_vec())),
            None => Ok(None),
        }
    }
    fn stage(&mut self, candidate: &Candidate) -> Result<()> {
        let bytes = disk::encode(candidate)?;
        let decoded = disk::decode(&bytes)?;
        self.point(Phase::BeforeStage)?;
        let (current, pending) = self.layout()?;
        Self::predecessor(current.as_ref(), &decoded)?;
    ...
    fn publish(&mut self, candidate: &Candidate) -> Result<()> {
        ...
        let (current, pending) = self.layout()?;
        Self::predecessor(current.as_ref(), &staged.candidate)?;
        let pending = pending.ok_or(Error::RecoveryRequired)?;
        if !staged.candidate.matches(candidate)
