...
- State the finding in the title, with its number when there is one. Do not reuse a title formula across notes, such as “What X’s N measures”.
- Start with the finding. Do not open with a paragraph that lists the questions the note will answer; the table of contents already shows the path.
- Write each limit once, in the note’s own terms, next to the result it limits. Do not paste a stock negative definition across notes, such as “It is not a subscription price, a production invoice, or a guarantee.”
- In a source list, say what each source is cited for (“Cited for the 47 to 56 step for Grok Build”). Do not write that a source “owns” a claim.
- Write a figure caption as one sentence that states the point of the article it illustrates. Do not describe what the illustration is not.

### Chart takeaways and other sentence templates

- Print a difference between two percentages in points (“4.13 points lower”), never as a percent.
- Write a cost multiple as “N× as much”, never “N× less than”.
- In a tie, compare within the tied group. Do not call whichever co-leader sorts first “the leader”.
- Do not start a sentence with a numeral. Spell out counts under 10, and write a range’s unit once (“from 4.84 to 57.62 index points”).
- Before merging a template change, render it against every published cohort and read the output. Ties, single-system charts, lower-is-better units, and missing costs all occur in the live data.
