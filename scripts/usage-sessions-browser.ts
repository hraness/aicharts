...
export async function verifyUsageSessions(browser: Browser, baseUrl: string): Promise<void> {
  ...
  for (const [name, width, colorScheme] of [["desktop", 1440, "light"], ["mobile", 390, "dark"]] as const) {
    ...
    try {
      ...
      await richExplorer.getByLabel("Metric", { exact: true }).selectOption("token-size-p95");
      ...
      invariant(await richExplorer.locator('option[value="hour-of-day"][disabled]').count() === 1, "Hour-of-day grouping must be refused without a declared time zone.");
      await richExplorer.getByLabel("Metric", { exact: true }).selectOption("observed-turn-count");
      await richExplorer.getByText("do not provide this kind of observation", { exact: false }).waitFor();
      await richExplorer.getByLabel("Metric", { exact: true }).selectOption("token-size-p95");
      await richExplorer.getByLabel("Session", { exact: true }).selectOption({ index: 1 });
      ...
      await richExplorer.getByLabel("Session", { exact: true }).selectOption("*");
