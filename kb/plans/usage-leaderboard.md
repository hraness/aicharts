...
- **Status:** Partial: browser registration, dormant server boundary, fresh-authentication coordinator and internal pairing lifecycle delivered. Browser custody and explicit live-account approval source are implemented; public integration, device enrollment and profile extensions remain unimplemented.
- **Depends on:** 0
...
- **Status:** Partial source only: local Worker tooling and numeric-only R2 staging implemented and locally runtime-tested. Authenticated admission remains unimplemented and disabled.
- **Depends on:** 0, 1
