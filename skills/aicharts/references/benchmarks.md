...
One benchmark command makes one catalog request and, only for a valid charted selection, one dataset request. Each response is at most 2 MiB; the benchmark retrieval has a 20-second deadline, at most 256 catalog entries and 2,048 points. The benchmark operation rejects credentials in source links, redirects, non-JSON, unexpected schema, invalid numbers, duplicate IDs and catalog/dataset mismatches. It does not fetch source links, run content, open local usage files, cache benchmark data, retry automatically or upload. Errors are fixed codes; response bodies and transport diagnostics are not printed. Bounds deliberately fail closed when the public distribution outgrows the reviewed contract.

Standalone successful reads can add optional support-protocol discovery on stderr using shared local invitation preferences. JSON stdout stays unchanged. Imported `run` calls remain quiet. Explicit `support` commands need no benchmark request. A due offer can suggest a local Git email; discovery does not read it. See [support.md](support.md) for consent, cadence and child-process controls. The shared runtime ships with this directory and requires no dependency installation.
