...
2. OpenRouter's public models API supplies a bounded 90-day release radar for the providers already represented by the site. It keeps text-output models with tool support, filters aliases and hosted variants, and records OpenRouter's listing timestamp as discovery metadata—not as a claimed release date. The same checked snapshot retains a durable `observedListings` ledger, so model-card recency does not disappear when an entry ages out of the bounded radar.
