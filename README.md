...
The [`data-refresh.yml`](.github/workflows/data-refresh.yml) workflow checks first-party release sources and OpenRouter discovery hourly, off the top of the hour. It refreshes Terminal-Bench 4, Terminal-Bench-Science 0.1, and direct DeepSWE evidence every four hours, then adds the heavier Artificial Analysis import to one daily full run at 10:43 UTC. Manual runs can select release-only, benchmark-only, or full refreshes; the legacy `discovery` mode remains a combined non-AAI alias. Poll-metadata-only checks remain visible in Actions without creating a data pull request. It treats every source as a separate failure domain:
