...
4. The workflow atomically updates only [`data/coding-agents.json`](data/coding-agents.json) and [`data/model-release-radar.json`](data/model-release-radar.json), runs the full project check, opens a dedicated automation pull request, explicitly dispatches required CI on its head commit, and enables squash auto-merge. It never bypasses the repository's protected-branch rules.
