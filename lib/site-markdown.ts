...
export function homeDocumentModel(
  ...
  return {
    ...
    paragraphs: [
      site.description,
      `The homepage compares capability with cost or output tokens on the ${intelligence.benchmark.name} v${intelligence.benchmark.version} Pareto frontier. Both resource views use the identical ${intelligence.selection.positiveCostRecordCount}-configuration positive-cost cohort. Inspect a point to see the model, effort setting, and exact values. A short row of recent Index listings sits above the chart; the source-backed release radar follows the chart. Those rows use the checked snapshot and reviewed discovery evidence, not invented scores.`,
...
function datasetMarkdown(snapshot: CodingAgentSnapshot): string {
  ...
  return joinMarkdown([
    ...
    "",
    "The standalone Terminal-Bench 4 owner cohort remains separate from the Terminal-Bench 4 component reported inside the Artificial Analysis Coding Agent Index. Every standalone TB4 JSON row retains the model, agent, agent version, effort, accuracy, 95% confidence interval, trials, cost, tokens, duration, and pinned source files.",
    "",
    ...
    "",
    `The source is the public Coding Agent Index v1.5 [${snapshot.source.name} coding-agents comparison](${snapshot.source.url}), composed of DeepSWE v1.1, Terminal-Bench 4, and SWE-Atlas-QnA. AI Charts retrieved this snapshot on ${formatRetrievedAt(snapshot.source.retrievedAt)}. The site checks for a new source snapshot daily. The displayed retrieval time changes only when a validated snapshot is stored.`,
    "",
    ...
    "",
    `For AA Index versus mean API cost see [highest AA Index and lowest cost pick different agents](${absolute(blogArticlePath("aa-index-cost-coding-agents"))}). For whether classified open-weight rows sit with those leaders see [open models closed SemiAnalysis composites, not this table](${absolute(blogArticlePath("open-models-coding-agent-benchmarks"))}). For how a cheaper model changed one daily news page see [GPT-5.6 Luna made one daily news page cost about $0.10](${absolute(blogArticlePath("small-models-have-arrived"))}). For the acceptance funnel, cost, tokens, and remaining miss rate behind the leading scientific score see [what Terminal-Bench-Science’s 30% result measures](${absolute(blogArticlePath("terminal-bench-science"))}). For why a public-suite high score still needs a holdout see [why a coding-agent high score still needs a holdout](${absolute(blogArticlePath("coding-agent-score-holdouts"))}).`,
    "",
    "## All configurations",
    "",
    ...
    `- The current v${currentIntelligence.benchmark.version} Intelligence snapshot is checked every four hours and the coding-agent snapshot daily; neither is a real-time mirror. The historical v4.1.1 snapshot is frozen. Use the relevant version and retrieval timestamp when citing a value.`,
    "",
    "## Dataset links",
    "",
    `- [Download current Intelligence v${currentIntelligence.benchmark.version} JSON](${absolute("/data/artificial-analysis-intelligence-v4-3.json")})`,
