...
import { parseStatsPublicReply, parseStatsPublicSearch, statsPublicStatus, STATS_PUBLIC_MEDIA, STATS_PUBLIC_URL, type StatsPublicError, type StatsPublicReply } from "./stats-public";

export interface StatsPublicDependencies {
  available(): boolean;
  query(request: Request, range: StatsRange): Promise<unknown>;
}
function send(request: Request, reply: StatsPublicReply): Response {
...
const failure = (request: Request, error: StatsPublicError) => send(request, { schemaVersion: 2, ok: false, error });
  ...
  return async (request: Request): Promise<Response> => {
    try {
      ...
      if (success?.ok === true) {
        ...
        return reply === null ? failure(request, "unavailable") : send(request, reply);
      }
      const absent = privateDaysSnapshot(query.result, ["ok", "error"]);
      if (absent?.ok === false && absent.error === "limit") return failure(request, "range_too_large");
      return absent?.ok === false && (absent.error === "not_enrolled" || absent.error === "not_started")
        ? failure(request, absent.error) : failure(request, "unavailable");
