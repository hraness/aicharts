  ...
  return async (request: Request, input: unknown): Promise<PrivateDaysTransportOutcome> => {
    try {
      ...
      const result = await pairingHttpWork<PrivateDaysTransportOutcome>({ ...effects, now: sample }, PAIRING_HTTP_CLIENT_MS, registerLifetime, failed, () => { outstanding--; }, async work => {
        ...
        try {
          guard();
          if (response.status !== 200 || response.url !== PRIVATE_DAYS_HTTP_URL || response.redirected
          ...
          const length = privateDaysHttpLength(response.headers, PRIVATE_DAYS_HTTP_RESPONSE_BYTES);
          reading = true;
          const bytes = await pairingHttpBody(response.body, PRIVATE_DAYS_HTTP_RESPONSE_BYTES, length, work);
          guard();
