...
import { parseStatsPublicSearch, STATS_PUBLIC_MAX_BYTES, STATS_PUBLIC_MEDIA, STATS_PUBLIC_URL } from "./stats-public";
...
test("invalid or private service failures become a fixed safe response", async () => {
  for (const result of [null, { kind: "authentication_required", email: "PRIVATE_CANARY" }, { kind: "query", result: { ok: true, value: { ...value, dayCount: 2 } } },
    { kind: "query", result: { ok: false, error: "PRIVATE_CANARY" } }]) {
...
test("client checks range, revision, media, length and HTTP envelope consistency", async () => {
  ...
  const fetcher = (body: unknown, status = 200, headers: Record<string, string> = {}) => (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    return new Response(JSON.stringify(body), { status, headers: { "content-type": STATS_PUBLIC_MEDIA, ...headers } });
  }) as typeof fetch;
  const read = (f: typeof fetch) => readPrivateStats(range.firstUtcDay, range.dayCount, new AbortController().signal, f);
  expect(await read(fetcher(good))).toEqual(good);
