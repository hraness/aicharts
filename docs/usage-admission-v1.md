...
There is no public receipt-import command. The owned authenticated transport must supply terminal bytes; parsing a file or caller-supplied receipt cannot establish acceptance. Explicit conflict reconciliation, ungating, operational device replacement and transport activation are not implemented by this ledger API.

## Native HTTPS transport

`crates/aicharts-cli/src/upload/https.rs` implements the sealed sender port using exact `ureq` 3.4.1 with only its Rustls feature. Production authority is fixed to `https://usage.aicharts.io/v1/batches`, with hostname verification, SNI and the pinned WebPKI root bundle. Ambient proxies, redirects, cookies, decompression, connection reuse and automatic retries are disabled. Alternate roots, resolver and endpoint exist only under `cfg(test)`. The type has no public arbitrary-token constructor or enrolled production constructor, and no CLI command calls it. Native credential custody and enrollment remain required before activation.

One exchange sends the exact bound batch, capped at 82,024 bytes, with a sensitive bearer header and fixed batch/journal media types. The CLI also pins `log` 0.4.34 as a runtime dependency with [`max_level_off`](https://docs.rs/log/0.4.34/log/#compile-time-filters) in every build profile, including tests. This disables dependency logging macros at compile time: `ureq-proto` can otherwise trace serialized Authorization and body bytes despite the header's sensitive Debug marker. The product installs no logger. A synthetic canary first proves an enabled trace sink receives a directly delivered record, clears it, then requires zero records during a real verified local TLS exchange and completed cleanup.

Success requires HTTP 200, one exact `application/vnd.aicharts.usage-journal-v1` Content-Type and one canonical positive Content-Length no larger than 67,744 bytes. Response headers are capped at 16 KiB and 64 fields. Transfer-Encoding, Content-Encoding, Location, Set-Cookie, Trailer and Upgrade are refused. All chunked and close-delimited bodies fail, including a chunked journal missing its final CRLF. An accepted body must reach the Content-Length message boundary with exactly the declared bytes; this is framed HTTP EOF, not peer socket EOF. The adapter drops the response and connection without waiting for FIN or draining an error body. The sender still validates the complete journal against its frozen batch before settlement.

The existing Worker adapter constructs its terminal response from a fixed `Uint8Array`. [Cloudflare's Response contract](https://developers.cloudflare.com/workers/runtime-apis/response/#set-the-content-length-header) derives Content-Length from fixed-length values; manually setting that header is ignored. This supports the source contract, but the actual deployed edge must still demonstrate an accepted Content-Length response before native upload qualification. An incompatible edge response retains the flight.

The monotonic acceptance budget is 45 seconds. DNS and request-header budgets are 3 seconds, connection 5 seconds, response headers 40 seconds, and each body stage 5 seconds, each capped by the remaining total. Deadline checks around blocking reads and after cleanup refuse late success. Only one process-wide OS DNS lookup may be outstanding; a timed-out lookup retains its permit until the OS returns and cannot start HTTP work. At most 16 returned addresses reach the connector. OS resolution and blocking socket/TLS calls are not preemptible and may return after their configured timeout; this is an acceptance deadline, not a hard wall-clock completion guarantee.

HTTP 401, 409 and 503 map to fixed unauthorized, blocked and unavailable errors; other non-200 statuses refuse the response. Errors and lost replies never settle or discard the batch, and the adapter performs no retry. A later explicit replay sends the retained bytes. Twenty focused local TLS cases and strict CLI Clippy passed, including the canary, wrong roots/hostname, truncation, delayed reads, DNS permit retention, dropped replies and socket cleanup. They establish synthetic adapter behavior, not live Accounts authority, device enrollment, edge compatibility or remote recovery.
