...
AI Charts contains server and Worker adapters for the four [pairing transport operations](usage-pairing-transport.md). The default authentication resolver loads the Vercel transport behind the independent, disabled pairing flag. `createPairingCoordinator()` composes the request-owned OIDC verifier with the Worker handler, but the default Worker entrypoint remains unavailable. The fixed target, `https://usage.aicharts.io/internal/pairing`, is planned configuration, not an observed live endpoint. [Usage identity and activation](usage-identity.md#device-enrollment-boundary) owns the guarded browser start, approval and callback contracts and their separate live acceptance criteria.
...
Each resolved intent can carry a synchronous current-authority predicate. The production resolver supplies the captured configuration, cookie-secret and abort checks; the coordinator and transport work guards separately enforce their clock bounds. The transport checks the predicate before and after synchronous context capture and through every work guard, including after lifetime registration. Closure during request setup prevents the later fetch; an already dispatched request remains under its owned cleanup. This per-intent predicate uses the existing shared factory rather than creating independent capacity for every browser request.
