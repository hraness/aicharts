...
| `/coding` | Coding-agent comparison workspace | AI coding agent comparison, coding benchmark cost and token use | Keep the original source-specific chart and axes accessible. Name each model, harness, and effort setting. Keep the Coding Agent Index v1.5 Terminal-Bench 4 component separate from the standalone owner cohort. |
...
| `/models` | Model identity collection | AI model pages, Intelligence Index and coding observations | List every current model page in static HTML with the source snapshot date and crawlable card links. |
| `/models/[creator]/[model]/[profile]` | Model identity page | Named AI model Index, cost, and coding observations | Keep logo, name, Index/cost, and source links first. Coding profiles keep observed ranges and the Gateway identity. Index-only pages do not invent coding scores. |
...
2. Decision trade-offs: `AI model performance vs cost`, `AI model speed comparison`, `token use comparison`, and benchmark-specific cost or quality questions. The homepage's Intelligence efficiency view answers general-model score-versus-output-token and score-versus-cost questions using Artificial Analysis v4.3.2; the AA Index versus cost note answers a different question for the coding-agent snapshot with named configurations and its retrieval date. The open-models note answers whether classified open-weight rows sit with the current coding-agent AA Index leaders, and keeps SemiAnalysis era composites separate from that table. The small-model economics note uses French-Owen's reported $0.10 news-page cost to show how to test whether a cheaper model meets a workload's quality bar. The Terminal-Bench-Science note explains why a scientist-set 30% peak resolution is incomplete without cost and token trade-offs. The holdout note answers why a public-suite high score still needs hidden cases. The HarnessTax note answers what a same-model cost gap covers when a coding model changes harness on two public suites. The harness-design note answers which components inside one fixed coding-agent loop (planning, action space, and context management) move success or cost, and under which model and window conditions.
3. Evidence: exact benchmark and methodology searches such as MirrorCode, SWE-bench, Terminal-Bench, Terminal-Bench-Science, HarnessTax, harness component ablations, and questions about what a result does or does not establish.
...
- description: `A portal for AI model benchmarks and individual token usage: compare capability, cost, and speed, and inspect coding-agent usage locally.`;
