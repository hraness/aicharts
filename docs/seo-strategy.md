...
| --- | --- | --- | --- |
| `/` | Pareto charts, then task-oriented benchmark library | AI model comparison, AI agent comparison, AI benchmark charts | Lead with the visible Intelligence Pareto curve and original coding-agent charts. Keep the library secondary; offer task filters, named configurations, native units, uncertainty, and comparable cohorts. Never hide the primary charts in a disclosure. |
