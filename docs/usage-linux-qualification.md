...
The [13 September run 34777098764](https://github.com/hraness/aicharts/actions/runs/34777098764) used PR 215 merge `a9363bdeb55972497ea388fafb47c3bd4fddd593`, tree `e2ec8ebdbbed5609482dcb5ef3d02f302313b672`, on Ubuntu image `20260907.292.1`. Exact source/build checks, ELF/runtime validation and all 14 CLI smokes passed. The collector then refused with `notices_system_missing`; the runner reported `notices_incomplete` before its output-size check, archive assembly or installation. There is no successful qualification artifact from this run.

That summary does not distinguish package lookup, metadata checks, copyright parsing or common-license reads. The collector now records fixed system failure locations without changing admission predicates, query deadlines or equivalent-path lookup attempts. Renew qualification from the next protected-main merge after the complete integration gate passes. The cause of this hosted refusal and successful assembly still require actual evidence.
