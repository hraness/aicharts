...
The [13 September run 34785498690](https://github.com/hraness/aicharts/actions/runs/34785498690) used PR 218 merge `0376373595942d35991185dfc76db69e0b06b399`, tree `11a7b1fae8d7fa3749a4716d52c03a0c9c20de56`, on Ubuntu image `20260907.292.1`. Exact source/build checks, ELF/runtime validation and all 14 CLI smokes passed. The collector then refused with `notices_system_missing / package_query_terminated`; its one exact retry `34785695232` reproduced the same bounded refusal. There is no successful qualification artifact from these runs.

The bounded diagnostic identifies package-query termination without exposing command arguments or output. The collector keeps the fixed query environment and all attribution predicates, but raises each `dpkg-query` timeout to 30 seconds to accommodate cold package databases while retaining the qualification's overall deadline. Renew qualification from the repair's protected-main merge; successful assembly still requires actual evidence.
