...
Authenticated GitHub acquisition, selected-source approval and safe installation remain separate requirements. Linux ELF/runtime and notice qualification is described in [docs/usage-linux-qualification.md](usage-linux-qualification.md); the tag-triggered immutable publication and OIDC attestation path is described in [docs/usage-release-publish.md](usage-release-publish.md) and is implemented but unexecuted. Neither this reader nor assembly enables authentication, collection, native credential custody, enrollment, uploads or updates.
