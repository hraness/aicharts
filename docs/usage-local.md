...
The 13 sender orchestration cases establish local retry/settlement behavior with synthetic authority and ledger reopen. The 20 HTTPS cases use local synthetic TLS and cover certificate/hostname rejection, framing, deadlines, DNS ownership, lost replies, cleanup and a live test logger that receives no exchange records. Run them with `cargo test --locked -p aicharts-cli upload::`. These results do not qualify live enrollment, actual edge framing or process-death recovery. No sending command is enabled; credential custody and enrollment must supply the sealed construction authority before that can change.
