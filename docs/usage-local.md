...
Without `--once`, the process prints each local collection result and sleeps between passes. Exhausted ledger contention or a changing source snapshot reports a fixed error and waits the same interval; other failures stop the process for supervisor-visible recovery. This runner is local persistence only: `uploaded: false` describes its own invocation. It does not send pending records. Use the enrolled `upload` command or the [bounded sync pass](#collect-and-publish-in-one-bounded-pass) to transmit accepted numeric measurements.
...
Unchanged file metadata skips source parsing. Changed sources are read again from the beginning; `--rescan` forces this for unchanged files too. `linesRead`, `bytesScanned` and `sourcesSkipped` describe that invocation. `sourcesDeferred` counts whole sources left for a later pass because their snapshots changed or their JSONL tails were unfinished. `sourcesConflicted` separately counts irreconcilable retained history. This is a source-snapshot checkpoint, not byte-tail resumption. Metadata skips are not tamper evidence. The ledger also validates its bounded numeric state on open, so a no-change scan still performs local integrity work.

Legacy collection saves stable, complete source snapshots in bounded atomic waves: at most 2,048 changed sources, 256 MiB of selected snapshot bytes or 100,000 measurements per wave. Each wave verifies sources again before committing checkpoints, deduplicated measurements and pending candidates at the current ledger revision. An observed source change discards that source's entire staged scan while stable siblings can commit. An unfinished JSONL tail also defers the entire source; no part of that scan advances its retained checkpoint or numeric history. A pass with every source deferred succeeds with zero updates. Deferred sources are reconsidered only on the next collection pass. Attempted snapshot bytes remain included in `bytesScanned`.

Stable malformed input, invalid state, stale concurrent writers and exceeded limits still fail the affected wave; earlier committed waves remain durable. JSONL snapshots require a final newline. Devin ATIF sources must parse as complete JSON documents. Retained measurements survive source omission and supported rewrites through dominance merging: older observations cannot reduce totals, and irreconcilable history is counted separately in `sourcesConflicted` while other sources continue. Automatic rotation migration, explicit deletion and decreasing corrections are not implemented.
...
Each bounded commit wave remains atomic; earlier committed waves remain durable if a later wave fails. Recovery from a corrupt ledger
...
A malformed complete record fails the scan with a fixed error. Legacy persistent collection defers an entire source with an unfinished tail; reindex still rejects it. Explicitly enabled prefix collection defers only the suffix as described above. Retry after the source writer completes; rejected imports advance no checkpoint or server state. Standard error never includes source content or paths. Internal bugs are still possible; a passed test suite is not a security audit.
