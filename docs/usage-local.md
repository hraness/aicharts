...
Only a synthetic transport implements this port. Its request contains owned numeric bytes, never a source path, ledger handle, polling secret or namespace key. A sticky journal byte limit rejects overflow even if an adapter ignores an append error. The seam performs one synchronous exchange; actual DNS, TLS, deadlines, cancellation and cleanup require separate transport qualification. The 13 focused `cargo test -p aicharts-cli --locked upload::` cases establish local orchestration with synthetic authority and ledger reopen, not process-death recovery or live upload. No public command or dependency is added.
