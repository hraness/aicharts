...
The dormant Rust [terminal enrollment codec](usage-terminal-enrollment.md) shares literal request and reply vectors with TypeScript. It checks canonical bytes, original pairing lifetime, explicit account choice and the complete reservation and receipt without a network or credential-store dependency. Run its focused cases with `cargo test --locked -p aicharts-cli --bin aicharts -- enrollment::`. The codec is the input to a later native enrollment transport and custody join.
