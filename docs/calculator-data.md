...
The own-hardware path prices straight-line depreciation plus electricity. Depreciation spreads the purchase price, less the resale-value knob's residual (default 0%), over the useful-life knob's months. Electricity multiplies rated GPU power draw by duty-cycle hours and the electricity knob's rate; the model assumes flat-rate grid supply with no on-site solar or other self-generation. The page also charts cumulative depreciation plus electricity at quarter marks of the useful life (`usefulLifeCheckpointMonths`) against the rented total over the same months, so ownership cost reads as an accrual, not one monthly bar.

Defaults price the DeepSeek path at the blended window (peak-hour weighted) and size fleets on the 24/7 duty cycle; the window and duty-cycle controls still cover off-peak, peak, and the 40-hour power-user week.
...
Admission checks real calendar dates and unique subsidy plan names. Automated refresh refuses older source retrieval instants, an older EIA reporting month, changed source identities, or edits to the rental methodology, peak-hour policy, and curated list-fallback date. Offset timestamps are compared as instants. A refusal leaves the last validated snapshot intact. Reviewed electricity preset dates, list-fallback documentation dates, and subsidy method dates all contribute to the calculator's discovery modification date.
...
The scenario model uses binary64 arithmetic for fractional rates and assumptions. Fleet sizing rounds up to a whole unit with the existing eight-epsilon allowance for floating-point noise near an integer, and retains at least one unit. Integer-domain properties independently check the fleet ceiling after hours-to-seconds conversion and calculate electricity from an exact watt-hour/cents rational reference. These tests support the scenario model; they do not establish exact billing or prove that cited throughput measurements predict a reader's workload. The [product assurance map](product-assurance.md) records the adjoining publication and companion boundaries.
