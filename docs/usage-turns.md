...
Each daily cohort also exposes `averageTurnLength`. Its `runtimeMs` is an exact `{ numerator, denominator, basis: "provider_reported_runtime_ms", coverage: "partial" }` ratio when at least one turn has qualified runtime, otherwise `null`. `tokens` and `toolCalls` remain `null`: the reader has no complete token population or dispatched-tool evidence. The nested `observedSubtotals` are the partial response-token and requested-call ratios above, not replacements for those unavailable complete metrics.
