...
Operating-system attempt and credential-reference persistence, the sealed production constructor, live Accounts authority, actual edge framing and process-death enrollment recovery remain unfinished. The private sequencing code is source-level state-machine coverage only; it does not connect custody to enrollment, create a provider session, establish a public service or activate product behavior. The separately admitted disposable macOS Keychain qualification proves one fresh file-based vault fixture and locked-access refusal, but it does not qualify those remaining joins or arbitrary restore. Follow [Usage Worker boundaries](usage-worker.md#validation-and-activation) and the [private Cloudflare procedure](usage-cloudflare-qualification.md) for their separate gates.
