...
The dormant terminal enrollment codecs, native HTTPS adapter and Worker HTTP factory connect six terminal operations to the existing pairing and account Durable Objects. A private attempt record and persistence core retain local observations separately from those exchanges. The private custody handoff validates the original typed pairing or namespace secret before advancing only nonsecret progress. Reference-store instance methods now reach the private macOS backend, while public construction remains closed. No production caller or CLI command activates this join. This source adds no public Worker route, production custody constructor, browser approval flow or upload authority.
...
The production join of attempt and credential-reference persistence, the sealed production constructor, live Accounts authority, actual edge framing and process-death enrollment recovery remain unfinished. The private sequencing and facade code does not create a provider session, establish a public service or activate product behavior. The [disposable macOS Keychain qualification](../crates/aicharts-custody/README.md#validation-and-qualification) now passes through the exact facade methods with pairing and namespace records, including reopening, lost-reply reconciliation, locked-access refusal and owned-fixture cleanup. That receipt covers the explicit fresh vault on this host; default User-domain keychain selection, the combined live enrollment path, CLI and LaunchAgent behavior, and arbitrary recovery remain unqualified. Follow [Usage Worker boundaries](usage-worker.md#validation-and-activation) and the [private Cloudflare procedure](usage-cloudflare-qualification.md) for their separate gates.
