...
`settle_response` revalidates a checked response against a fresh observation before changing facts. Initialize, poll, confirm, reserve and enroll successes advance their corresponding facts and retire their completed flight. A namespace success pins the typed namespace `RecordIntent` commitment while retaining the dispatched flight for the later private custody transition. Checked domain errors remain fixed failure facts; transport and ambiguous outcomes retain the flight for explicit reconciliation. Request bodies, response bodies, proof preimages and namespace keys never enter the canonical record. Focused `cargo test --locked -p aicharts-cli --bin aicharts enrollment::attempt` covers these transitions alongside the record, storage and macOS adapter cases; no production constructor, command, credential-reference store, network caller or activation path is connected.
