...
The dormant [Usage Worker primitives](usage-worker.md) implement internal pairing transitions through terminal confirmation and numeric-only R2 staging. The server-only pairing coordinator consumes immutable Suite Accounts v0.6.0 fresh completion internally. It seals the intent, attempt, browser nonce and context capability in a canonical, versioned 190-character context, then records only the SDK-verified account, signed authentication time and earliest evidence expiry against that exact durable attempt. It returns the SDK continuation only after an exact successful recording response. It never returns a verification DTO for browser code to forward and never invokes approval, confirmation or enrollment.

The factory's explicit `startPairingAuthentication` and `completePairingAuthentication` methods are not connected to the public auth handler. No production intent resolver or authenticated Vercel-to-Worker transport is installed. Start requires the exact canonical GET start URL without query parameters, same-origin fetch metadata and no conflicting Origin header; its intent and browser nonce must come from trusted product orchestration, not copied request JSON. The future approval flow must generate and retain the browser nonce in protected cookie custody and independently check CSRF and the live account. Encoding a nonce is not that approval flow.
