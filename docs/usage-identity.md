...
The dormant [coordinator verifier](usage-coordinator-verifier.md) implements the expected Vercel workload identity policy with request-owned work and a bounded completed-public-key cache. It has no production caller or token-acquisition path. Synthetic unit checks do not establish live provider compatibility, real HTTP Worker lifetime behavior, fleet-wide abuse controls or activation readiness.
