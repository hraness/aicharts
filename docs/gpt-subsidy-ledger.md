...
The v2 public snapshot publishes the measured API-retail-equivalent value of each trailing seven-day UTC window. It does not project that value into a month. Historical observations retain `accountAttribution.status: "unavailable"`, zero coverage, a null observed-account count, and a null `subscriptionAdjustedMultiple` because session files cannot be attributed account by account.

The latest 31-day summary has a separate account-plan comparison, and every seven-day observation receives coverage from the same clipped interval calculation. `scripts/enrich-gpt-subsidy-attribution.ts` reads the private account ledger after token measurement and publishes only coarse whole-percentage lower-bound coverage and distinct counts; positive coverage below one percent is published only as `<1%`, and coverage is never labeled complete. The observed-account count remains available before the optional quota ledger starts. When quota observations exist, a fingerprint is included in the separate Pro-status lower bound only when it has at least one same-key app-server observation in the summary period and every bucket in every overlapping observation reports `planType: "pro"`; one null, mixed, or non-Pro report excludes it. This is provider-reported plan-status evidence, not billing verification. The API-equivalent value divided by that lower-bound count times the checked plan price is an upper bound. The publisher never extrapolates account count from partial coverage and omits only the plan comparison when its evidence is unavailable. Raw fingerprints, quota buckets, provider limit IDs, and individual intervals are never published. Per-observation `subscriptionAdjustedMultiple` remains null.

The attribution calculation has its own checked manifest so token measurement revision `2026-08-25.3` and the installed Tokscale adapter remain unchanged. An hourly sampler can miss an account used entirely between samples, and current allowance state still cannot be joined to historical token use account by account. The comparison is a normalized plan-price bound, not audited subscription spend or proof that all measured usage came from subscriptions.
