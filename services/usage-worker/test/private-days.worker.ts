...
describe("dormant private imported days", () => {
  ...
  test("all providers retain exact imported totals with reasoning counted only once", async () => {
    ...
    success(await upload(device, batch(device, 1, [{ id: 1, input: 10n, cache: 7n, output: 5n, reasoning: 5n },
      ...
      { id: 3, provider: 3, input: 9n, cache: 4n, output: 3n }])));
    ...
    expect(result.days[1]).toEqual({ utcDay: DAY, codex: totals(1, "22", "5"), claudeCode: totals(), devin: totals(1, "16", "3") });
